...
public:
	...
	void output(void(*out)(const T&)) const;				// 输出队列数据
...
template<class T>void Queue<T>::output(void(*out)(const T&)) const
{
	const_iterator i;
	int j;
	for (i = hd->next,j=0; i != tl; i = i->next,j++)
	{
		out(i->data);
		std::cout << ' ';
		if (9 == j % 10)
...
{
	clear();
	for (const_iterator i = begin->next; i != end; i=i->next)
		in(i->data);
}
